...
buttonList.forEach((element) => {
  ...
  buttons.setAttribute("button-id", element);
  buttons.innerHTML = element;
  buttons.addEventListener("click", buttonClick);
  buttonDOM.append(buttons);
});

//Button tıklama fonksiyonu ile filterleme
function buttonClick() {
  if (this.getAttribute("button-id") == "All") {
    return createMenu(menu);
  } else {
    let selectedMenu = menu.filter(
      (element) => element.category == this.getAttribute("button-id")
    );
    return createMenu(selectedMenu);
  }
}

//Menü elemanlarını ekleme
